...
public sealed class PokeparkScene(ISpriteService sprites, PokeparkSpriteService walking)
{
    ...
    private sealed class ResidentRoute(ParkWander wander)
    {
    ...
    {
        var scale = Math.Max(width / ParkMap.Width, height / ParkMap.Height);
    ...
    public ParkResidentState GetResidentState(int index, long time)
    {
        ...
        if (!_routes.TryGetValue(key, out var route))
        {
            ...
            _routes[key] = route = new(new ParkWander(ParkMap.For(EnvironmentIndex), allowed, preferred,
                unchecked((int)(seed + (uint)EnvironmentIndex * 7919)), radius: 12));
        }
        var animated = walking.GetFrame(mon.Species, mon.Form, mon.Shiny, 0, 0) is not null;
